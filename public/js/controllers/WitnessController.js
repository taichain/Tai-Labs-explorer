...
var beginDraw = function(labelList, dataList){
//   console.log("begin draw");
    var config = {"type":"bar",
    "data":{
        "labels":labelList,//["6 day","5 day","4 day","3 day","2 day","1 day","today"],
        "datasets":[
            {"label":"Day Rewards",
            "data":dataList,
            "fill":false,
...
angular.module('BlocksApp').controller('WitnessController', function($stateParams, $rootScope, $scope, $http, $location) {
      ...
      $http({
        ...
        if (data.error)
          ...
          if(_page==0 && data.blocks.length>1){
            var startId = 0;
            var endId = 0;
            for(var j=0; j<data.blocks.length-1; j++){
              if(data.blocks[j].timestamp-data.blocks[j+1].timestamp > 500){
                startId = j+1;
                endId = startId;
              }else{
                endId++;
            ...
            if(endId == startId){
              $scope.blockTime = "not effective time"
            }else{
              var totalTime = data.blocks[startId].timestamp - data.blocks[endId].timestamp;
              $scope.blockTime = totalTime/(endId-startId);
            }
    ...
    var historyInfo = function(){
        $http({
            ...
            var labelList = [];
            var dataList = [];
            var docs = respData.docs;
            var fromDayTime = respData.fromDayTime;
            ...
            for(var i=0; i<=dayBefore; i++){
                ...
                for(var j=0; j<docs.length; j++){
                    if(docs[j].timestamp>startTime && docs[j].timestamp<endTime){
                        mineBlockNum++;
                    }else{
                        //t break;
                    }
                }
                dataList.push(mineBlockNum*0.3375);
                
            }
            
            beginDraw(labelList, dataList);
          });
    }
