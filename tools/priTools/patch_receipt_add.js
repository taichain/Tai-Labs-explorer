            ...
            if(receiptData){
                ...
                for(k in receiptData.logs){
                    ...
                    if(logEvent.eventName.indexOf("Released(")==0){
                        ...
                        txData.value = etherUnits.toEther(logEvent.topics[2], 'wei');
                    }
                }
...
function updateBlockNumber(){
    var hashArr = [];
    var numberArr = [];
    Transaction.find({'gasUsed':null}, "hash blockNumber", function(err, docs){
        for(var i=0; i<docs.length; i++){
            hashArr.push(docs[i].hash);
            numberArr.push(docs[i].blockNumber);
        }
        
        for(var j=0; j<hashArr.length; j++){
            Transaction.update(
                {'hash':hashArr[j]}, 
                {$set:{'blockNumber':numberArr[j]+1}}, 
                {multi: false, upsert: false}, 
                function (err, data) {
                    if(err)
                        console.log(err);
