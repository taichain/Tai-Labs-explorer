...
var upsertAddress=function(miner, addrs){
    if(miner){
        ...
        Address.update({"addr":miner},
            {$set:{"type":2}, $inc:{"balance":337500000000000000}},
            {upsert: true},
            function (err, doc) {
                if(err)
                    console.log("err:", err);
...
var writeTransactionsToDB = function(blockData) {
    ...
    if (blockData.transactions.length > 0) {
        for (d in blockData.transactions) {
            ...
            if(txData.intxs){
                innerTxs = [];
                for(var k=0; k<txData.intxs.length; k++){
                    innerTxs.push({from:txData.intxs[i].from, to:txData.intxs[i].to, value:etherUnits.toEther(new BigNumber(txData.intxs[i].value), 'wei'), 
                    blockNumber:blockData.number, timestamp:blockData.timestamp});
                }
                txData.intxs = null;
            }
            ...
            if(receiptData){
                ...
                for(k in receiptData.logs){
                    ...
                    //deal with Released as internalTX
                    // if(logEvent.eventName.indexOf("Released(")==0){
                    //     txData.from = txData.to;
                    //     if(logEvent.topics.length>2){//index param
                    //         txData.to = "0x"+logEvent.topics[1].substr(26);
                    //         txData.value = etherUnits.toEther(new BigNumber(logEvent.topics[2]), 'wei');
                    //     }else{//param without index
                    //         txData.to = "0x"+logEvent.data.substring(26,66);
                    //         txData.value = etherUnits.toEther(new BigNumber("0x"+logEvent.data.substr(66,64)), 'wei');
                    //     }
                    // }
                }
