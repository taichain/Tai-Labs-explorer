...
function checkDiscreteBlockNumber(){
    	if(config3.checkEndBlockNumber=='latest'){
		config3.checkEndBlockNumber = web3.eth.blockNumber;
	}
	Block.find({'number':{$gt:config3.checkStartBlockNumber, $lt:config3.checkEndBlockNumber}}, "number").sort('number').exec(function (err, docs) {
        if(err){
            console.log(err);
            return;
        }
